/**
 * Online Search Provider Architecture & Abstraction
 * Decouples GTAR search consumers from provider-specific endpoints and parsing.
 */
...
export type OnlineSearchResult = OnlineChordResult
...
export type OnlineSheetResult = FetchedChordSheet
...
/**
 * Provider abstraction interface for online chord and song search.
 */
export interface OnlineSearchProvider {
  readonly id: string
  readonly name: string
  search(query: string): Promise<OnlineChordResult[]>
  fetchSheet(result: OnlineChordResult): Promise<FetchedChordSheet>
}

/**
 * Curated offline chord catalog with authentic chords & multiple versions
 */
export const CURATED_CATALOG: Array<{
  keywords: string[]
...
/**
 * Ultimate Guitar Online Search Provider
 * Implements OnlineSearchProvider targeting GTAR backend scraper endpoints.
 */
export class UltimateGuitarProvider implements OnlineSearchProvider {
  readonly id = 'ultimate-guitar'
  readonly name = 'Ultimate Guitar'

  async search(query: string): Promise<OnlineChordResult[]> {
  ...
  async fetchSheet(result: OnlineChordResult): Promise<FetchedChordSheet> {
...
const DEFAULT_PROVIDER_ID = 'ultimate-guitar'

/**
 * Provider registry ensuring deterministic, explicit provider selection without automatic failover.
 */
class SearchProviderRegistry {
  private providers = new Map<string, OnlineSearchProvider>()
  private activeId = DEFAULT_PROVIDER_ID

  constructor() {
    this.reset()
  }

  register(provider: OnlineSearchProvider): void {
    if (!provider || !provider.id) {
      throw new Error('Invalid provider: provider must have an id')
    }
    this.providers.set(provider.id, provider)
  }

  getActive(): OnlineSearchProvider {
    const provider = this.providers.get(this.activeId)
    if (!provider) {
      throw new Error(`Active provider "${this.activeId}" is not registered`)
    }
    return provider
  }

  setActive(providerOrId: OnlineSearchProvider | string): void {
    if (typeof providerOrId === 'string') {
      if (!this.providers.has(providerOrId)) {
        throw new Error(`Cannot activate unknown provider ID: "${providerOrId}"`)
      }
      this.activeId = providerOrId
    } else {
      if (!providerOrId || !providerOrId.id) {
        throw new Error('Invalid provider: provider must have an id')
      }
      this.register(providerOrId)
      this.activeId = providerOrId.id
    }
  }

  reset(): void {
    this.providers.clear()
    const defaultProvider = new UltimateGuitarProvider()
    this.providers.set(defaultProvider.id, defaultProvider)
    this.activeId = DEFAULT_PROVIDER_ID
  }

  getRegistered(): OnlineSearchProvider[] {
...
export function getActiveSearchProvider(): OnlineSearchProvider {
  return registry.getActive()
}

export function setActiveSearchProvider(providerOrId: OnlineSearchProvider | string): void {
  registry.setActive(providerOrId)
}

export function registerSearchProvider(provider: OnlineSearchProvider): void {
...
export function getRegisteredSearchProviders(): OnlineSearchProvider[] {
